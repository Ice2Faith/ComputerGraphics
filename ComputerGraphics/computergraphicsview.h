...
protected:
	...
	afx_msg void OnOperationTips();// 操作提示菜单
	afx_msg void OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags);// 键盘按键响应
	afx_msg void OnCartonSetting();// 动画设置菜单
	afx_msg void OnTimer(UINT nIDEvent);// 动画定时器响应
	afx_msg void OnDismodNone();// 不显示模型菜单
	afx_msg void OnLinemodBezieryspin();// 绕Y轴生成旋转体菜单
	afx_msg void OnShowOrgline();// 显示坐标线菜单
	afx_msg void OnHideOrgline();// 隐藏坐标线菜单
	afx_msg void OnLinemodBezierxspin();// 绕X轴生成旋转体菜单
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};
