...
protected:
	...
	afx_msg void OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags);// 一个废弃的菜单
	afx_msg void OnCartonSetting();// 动画设置菜单
	afx_msg void OnTimer(UINT nIDEvent);// 动画控制定时器
	afx_msg void OnDismodNone();// 不显示三维模型菜单
	afx_msg void OnLinemodBezieryspin();// 创建Bezier旋转体菜单
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};
